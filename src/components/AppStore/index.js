...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, filteredList: ''}

  OnSearchAppItem = event => {
    this.setState({filteredList: event.target.value})
  }

  getSearchedList = filteredListApps => {
    const {filteredList} = this.state
    const searchItems = filteredListApps.filter(eachAppObj =>
      eachAppObj.appName.toLowerCase().includes(filteredList.toLowerCase()),
    )
    return searchItems
  }

  getEachCategoryApps = activeTabId => {
    const filteredList = appsList.filter(
      eachAppObj => eachAppObj.category === activeTabId,
    )
    return filteredList
  }
  ...
  render() {
    ...
    const filteredListApps = this.getEachCategoryApps(activeTabId)
    const searchedList = this.getSearchedList(filteredListApps)
    return (
      <div className="appContainer">
        <h1 className="appStoreHead">App Store</h1>
        <div className="searchIconAndImag">
          <input
            type="search"
            onChange={this.OnSearchAppItem}
            placeholder="Search"
            value={filteredList}
            className="searchBar"
          />
        ...
        <ul className="EachTabNameUlContainer">
          {tabsList.map(eachTabItem => (
            <TabItem
              eachTabItem={eachTabItem}
              tabId={eachTabItem.tabId}
              updateClickedTabId={this.updateClickedTabId}
              activeTabIdIs={eachTabItem.tabId === activeTabId}
        ...
        <div>
          <ul className="eachAppItemUlContainer">
            {searchedList.map(eachAppItem => (
